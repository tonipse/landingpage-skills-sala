...
> **🔗 AGENCY X INTEGRATION (Layer-Klärung):**
> Dieser Skill ist die **Motion-QUALITÄTS-Ebene** (WARUM/Audit) — NICHT der Code-Generator.
> - **`scroll_animation_agent`** = WIE (GSAP/Motion-Code, fertige Patterns, Mobile-safe-Setup) → für den eigentlichen Build
> - **`design_motion_principles`** (dieser Skill) = WARUM + AUDIT (Prinzipien von Emil Kowalski/Linear, Jakub Krehel, Jhey Tompkins; prüft fertige Animationen auf AI-Slop)
>
> **Wann dieser Skill (nicht scroll_animation_agent):**
> - „Audit meine Animationen", „ist die Motion gut?", „review die Übergänge"
> - Vor LP-Launch: Motion-Slop-Check der gebauten Seite
>
> **Wann scroll_animation_agent:** „bau die Scroll-Animation", „mach den Hero animiert" → Code-Output.
> Bei LP-Build: erst `scroll_animation_agent` (bauen) → dann dieser Skill (auditieren).
